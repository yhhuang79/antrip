...
//			alert("default");
	...
	*/
	...
	function UpdateAlbum(){
		...
		var index=1;
		var isHasPic = false;
		$("#img_body").html("<ul id='carousel_scroll' class='jcarousel jcarousel-skin-tango'></ul>");
		var fixedValue = 20;
		...
		for(var i=0;i<g_tripMarkerArray.length;i++){
			...
			if(file==""){
				...
				if(typeof  ImageInfo!='undefined'){
					...
					$("ul[id*=carousel_scroll]").append("<li><img id=scrollimg_"+i+" exif='true' src='"+file+"' align='left bottom' class='class_imagesOfAlbum' onClick=\"window.open('"+file+"', '_blank');\" width='"+ewidth+"' height='"+eheight+"' onMouseOver=\"top.frames['tripMapFrame'].pointToMarkerOnMap("+i+")\"  style=\"width:'"+ewidth+"';height:'"+eheight+"';cursor: pointer;\" /></li>");
				}
			}
		}
		//alert(isHasPic);
		if(isHasPic == true){
			//alert('scroll');
			if(typeof  initScrollfun=='function'){
				initScrollfun();
			}
		}
