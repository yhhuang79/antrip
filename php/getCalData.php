	...
	//		$s .= "$(\"tr[id=speed_table]\").eq(0).html(\"<th scope='row'>\"+g_str_speed_table+\"</th>\");";
		//	$s .= "$(\"tr[id=checkin_table]\").eq(0).html(\"<th scope='row'>\"+g_str_checkin_table+\"</th>\");";
			...
			if($_unit!=null){
				...
				if($interval<$MinToMicromin || ($interval/$MinToMicromin)<=5){
					$unit = "g_str_s";
					$MinToMicromin = $MinToMicromin_s;
					$table_dis = round(($interval/$MinToMicromin)/100);
					$s .= "$('#radio1').attr('checked', 'checked');";
				}
				else if(($interval/$MinToMicromin)>=500){
					$unit = "g_str_h";
					$MinToMicromin = $MinToMicromin_h;
					$table_dis = round(($interval/$MinToMicromin));
			...
			for($i=0;$i<(count($_tripArray)-$dis);){
				...
				if($interval>$MinToMicromin){
					...
					$speed1 = $_tripArray[$i]->spd;
					$speed2 = $_tripArray[$i+$dis]->spd;

					$append_altitudecontent = ($alt1 + $alt2)/2;

					$append_speedcontent = ($speed1+$speed2)/2;
					//if($append_speedcontent=="."){
					//	$append_speedcontent = 0;
				//	}
					$append_checkincontent = $checkinNum;
					/*if($index!=0){
						$s .= ", ";
					}*/
				//	if($index%$table_dis!=0){
					//	$s .= "[' ',   0,  ".$append_speedcontent."  , ".$append_checkincontent." ]";
						//$s .= "$(\"tr[id=calculate_col]\").eq(0).append(\"<th class='c_table_font' scope='col'>&nbsp;</th>\");";
				//	}
					//else{
						$s .= ", [".$index.", ".$append_altitudecontent.", ".$append_checkincontent.", ".$append_speedcontent."]";
